...
# ========== دیتابیس فوق‌پیشرفته (رفع باگ‌ها) ==========
class UltraDatabase:
    def __init__(self):
        self.db = db
        self.default_settings = {
            ...
            "anti_spam": True,
            "spam_threshold": 3,      # کاهش یافته برای تشخیص بهتر
            "spam_action": "mute",
            "spam_duration": 300,
            "anti_raid": True,
            "raid_threshold": 5,
        ...
        self.polls = {}
        self.user_messages = defaultdict(lambda: deque(maxlen=50))  # برای ضد اسپم
        self._load_stats()
    ...
    def add_warning(self, group_id, user_id, reason):
        ...
        # بازگرداندن تعداد اخطارهای فعلی برای این گروه
    ...
    # ========== ضد اسپم (با شمارش دقیق) ==========
    def add_message(self, user_id):
        """ثبت زمان ارسال پیام برای تشخیص اسپم"""
        now = time.time()
        self.user_messages[user_id].append(now)
        # حذف زمان‌های قدیمی‌تر از 10 ثانیه برای جلوگیری از رشد بی‌نهایت
    ...
    def get_message_count(self, user_id, seconds):
        ...
        # حذف زمان‌های قدیمی‌تر از seconds
...
• ضد اسپم هوشمند (بیزین + شمارش پیام)
• ضد حمله و رید
• ضد لینک، فحش، تبلیغات
• کپچا پیشرفته
• سیستم سطح‌بندی
...
def backup_command(message):
    if not is_bot_admin(message.from_user.id):
        ...
        return
    try:
        data = {"stats": dict(udb.stats), "timestamp": time.time()}
...
def handle_new_member(chat_member_update: ChatMemberUpdated):
    ...
    if new.status == "member" and old.status in ["left", "kicked"]:
        ...
        if settings.get('anti_raid', True):
            ...
            if join_count >= settings.get('raid_threshold', 5):
...
def captcha_answer(message):
    ...
    if not captcha_data or captcha_data["group_id"] != group_id:
        return
...
def handle_message(message):
    ...
    # اگر پیام یک دستور ثبت شده باشد، نادیده گرفته می‌شود (قبلاً پردازش شده)
    if message.text and any(message.text.startswith(cmd) for cmd in command_handlers):
        return
    
    # ادمین‌ها و خود بات را نادیده بگیر
    ...
    if settings.get('anti_spam', True) and message.text:
        ...
        count = udb.get_message_count(user_id, 1)  # تعداد پیام در ۱ ثانیه اخیر
        threshold = settings.get('spam_threshold', 3)
        logger.debug(f"کاربر {user_id}: تعداد پیام در ۱ ثانیه = {count}, آستانه = {threshold}")
        if count >= threshold:
            ...
            try:
                ...
                pass
            return  # پس از اعمال جریمه، پردازش بیشتر متوقف شود
    ...
    if data.startswith("toggle_"):
        ...
        if toggle in toggles:
            ...
            if toggle in ["welcome", "captcha", "autodelete"]:
                bot.edit_message_text("🔰 تنظیمات پایه", chat_id, call.message.message_id, reply_markup=basic_settings_menu(gid), parse_mode='HTML')
            elif toggle in ["antispam", "antiraid", "bayesian"]:
                bot.edit_message_text("🛡️ تنظیمات ضد اسپم", chat_id, call.message.message_id, reply_markup=spam_settings_menu(gid), parse_mode='HTML')
...
if __name__ == "__main__":
    print("=" * 70)
    print("✨ ربات محافظ فوق‌پیشرفته Luffy Ultra Pro (نسخه بدون باگ) ✨")
    print("=" * 70)
    print(f"👥 ادمین‌ها: {ADMIN_IDS}")
    print("✅ ضد اسپم با شمارش دقیق پیام‌ها (آستانه پیش‌فرض: ۳ پیام در ثانیه)")
    print("✅ سیستم اخطار با قابلیت ثبت چندین اخطار")
    print("✅ تشخیص محتوای حساس و ضد اسپم بیزین")
    print("✅ لیست سیاه و سفید، پاسخ خودکار، یادآوری")
